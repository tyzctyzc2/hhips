...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_workbypaper` AS select `w`.`idwork` AS `idwork`,`w`.`workdate` AS `workdate`,`w`.`idproblem` AS `idproblem`,round((`w`.`usedtime` / 60),0) AS `usedtime`,`w`.`workdetail` AS `workdetail`,`w`.`workmark` AS `workmark`,`w`.`reason` AS `reason`,`re`.`reasonfactor` AS `reasonfactor`,`w`.`idstarreason` AS `idstarreason`,`sr`.`starreasonname` AS `starreasonname`,`sr`.`starreasonfactor` AS `starreasonfactor`,`pw`.`idpaper` AS `idpaper`,`p`.`problemchapterid` AS `problemchapterid` from ((((`work` `w` join `paperwork` `pw` on((`pw`.`idwork` = `w`.`idwork`))) join `problem` `p` on((`w`.`idproblem` = `p`.`idproblem`))) left join `starreason` `sr` on((`w`.`idstarreason` = `sr`.`idstarreason`))) left join `reason` `re` on((`w`.`reason` = `re`.`idreason`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-06-17 20:16:37
